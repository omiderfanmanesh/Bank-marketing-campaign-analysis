...
class BasedDataset:
    ...
    def categorical_feature_encoder(self, encoder_type, col):
        le = None
        le_name = None
    ...
    def categorical_feature_mapping(self, col, mapping_value):
        new_col = self.generate_new_column_name(col=col, prefix='mapping')
        self.df[new_col] = self.df[col].map(mapping_value)

    def encoder(self, enc, X_train, X_test=None, y_train=None, y_test=None):
    ...
    def do_encode(self, X_train, X_test, y_train, y_test):
        for col in tqdm(self._cfg.ENCODER):
            ...
            col = col.lower()
            enc, enc_name = self.categorical_feature_encoder(encoder_type=encode_type, col=col)
            if encode_type == EncoderTypes.LABEL:
                train_val = X_train[col].values
                test_val = X_test[col].values
                X_train[col], X_test[col] = self.encoder(enc=enc, X_train=train_val, X_test=test_val)
            else:
                X_train, X_test = self.encoder(enc=enc, X_train=X_train, X_test=X_test, y_train=y_train, y_test=y_test)
    ...
    def scaler(self, scl, train, test=None):
        scl.fit(train)  # Apply transform to both the training set and the test set.
        train_scale = scl.transform(train)
        if test is not None:
            test_scale = scl.fit_transform(test)

        return train_scale, test_scale, scl
